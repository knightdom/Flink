...
/**
 * Created by Flink - OrderTimeoutDetectExample
 *
 * @Author: Edgar Fang
 * @Date: 2023/1/9 23:12
 */
public class OrderTimeoutDetectExample {
    public static void main(String[] args) throws Exception {
        ...
        // 3 将模式应用到复杂数据流上
        PatternStream<OrderEvent> patternStream = CEP.pattern(orderEventStream.keyBy(data -> data.orderId), pattern);
    ...
    public static class OrderPayMatch extends PatternProcessFunction<OrderEvent, String> implements TimedOutPartialMatchHandler<OrderEvent> {
        ...
        public void processTimedOutMatch(Map<String, List<OrderEvent>> map, Context context) throws Exception {
            // 获取超时未支付的订单事件
            OrderEvent craeteEvent = map.get("create").get(0);
            // 创建侧输出流用于保存超时的模式
            OutputTag<String> timeoutTag = new OutputTag<String>("timeout") {};
            context.output(timeoutTag, "用户" + craeteEvent.userId + " 订单 " + craeteEvent.orderId + "超时未支付！");
        }
    }
}
